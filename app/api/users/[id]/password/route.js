...
export async function PATCH(request, { params }) {
  ...
  if (admin?.role !== 'COMPLIANCE_ADMIN') return NextResponse.json({ error: 'Admin access required' }, { status: 403 });
  ...
  if (!target || !resettableRoles.has(target.role)) return NextResponse.json({ error: 'User tidak ditemukan atau tidak dapat diubah dari menu ini.' }, { status: 404 });
