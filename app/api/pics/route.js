...
export async function GET(request) {
  ...
  const where={role:'BU_PIC',...(businessUnitId?{businessUnitId}:{})}; if(user.role==='BU_PIC')where.businessUnitId=user.businessUnitId;
...
export async function POST(request) {
  ...
  try { const pic=await db.user.create({data:{name:name.trim(),email:email.trim().toLowerCase(),phone:phone?.trim()||null,jobTitle:jobTitle?.trim()||'PIC',locale:locale||'id',businessUnitId,passwordHash:await bcrypt.hash(temporaryPassword,10),role:'BU_PIC'}}); await db.auditLog.create({data:{actorId:user.id,entity:'User',entityId:pic.id,action:'CREATE_PIC',detail:JSON.stringify({businessUnit:businessUnit.name})}}); return NextResponse.json({id:pic.id,name:pic.name},{status:201}); } catch { return NextResponse.json({error:'Email PIC sudah terdaftar atau data tidak valid.'},{status:409}); }
}
