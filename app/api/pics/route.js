...
export async function POST(request) {
  const user=await currentUser(); if(!['SUPER_USER','CORPORATE_GOVERNANCE'].includes(user?.role))return NextResponse.json({error:'Corporate Governance access required'},{status:403});
