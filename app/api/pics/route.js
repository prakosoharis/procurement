...
export async function GET(request) {
  const timing=startApiTiming('/api/pics');
  const user=await timing.measure('auth',()=>currentUser()); if(!user)return timing.apply(NextResponse.json({error:'Authentication required'},{status:401}));
  ...
  const pics=await timing.measure('db',()=>db.user.findMany({where,select:{id:true,name:true,email:true,phone:true,jobTitle:true,locale:true,businessUnitId:true,businessUnit:{select:{name:true,groupName:true,industry:true}}},orderBy:{name:'asc'}}));
  return timing.apply(NextResponse.json(pics));
}
