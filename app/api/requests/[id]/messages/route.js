...
export async function POST(request,{params}){
  ...
  if(user.role==='BUSINESS_UNIT_PIC'&&ticket.requesterId!==user.id)return NextResponse.json({error:'Akses ditolak.'},{status:403});
  ...
  if(user.role==='BUSINESS_UNIT_PIC'){const recipients=await db.user.findMany({where:{role:{in:['SUPER_USER','CORPORATE_GOVERNANCE']}},select:{id:true}});if(recipients.length)await db.ticketNotification.createMany({data:recipients.map(recipient=>({messageId:message.id,recipientId:recipient.id})),skipDuplicates:true});}
