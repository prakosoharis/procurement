...
export default function Actions({ sopId, version, publishedVersion, capabilities = {}, onDone }) {
  ...
  return <section className="repository-actions"><b>Available actions</b>{c.canEditDraft&&version.lifecycleState==='DRAFT'&&<EditDraft version={version} onDone={onDone}/>} {c.canSubmitDraft&&version.lifecycleState==='DRAFT'&&<button onClick={()=>confirm('Submit this draft for governance review?')&&call(`/api/governance/versions/${version.versionId}/submit`,{expectedState:'DRAFT'})}>Submit</button>} {c.canStartRefinement&&version.lifecycleState==='SUBMITTED'&&<button onClick={()=>call(`/api/governance/versions/${version.versionId}/refinement/start`,{expectedState:'SUBMITTED'})}>Start Refinement</button>} {c.canCompleteHumanRefinement&&version.lifecycleState==='REFINEMENT'&&<button onClick={()=>{const reason=prompt('Reason'),summary=prompt('Human-only refinement summary');if(reason&&summary)call(`/api/governance/versions/${version.versionId}/refinement/complete-human`,{expectedState:'REFINEMENT',reason,summary})}}>Complete Human-Only Refinement</button>} {c.canCreateRevision&&publishedVersion&&<button onClick={()=>{const reason=prompt('Revision reason');if(reason)call(`/api/governance/sops/${sopId}/revisions`,{sourceVersionId:publishedVersion.versionId,expectedState:'PUBLISHED',reason})}}>Create Revision</button>}<small>{message}</small></section>;
}
