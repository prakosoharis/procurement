...
async function scopedPosition(tx, actor, positionId) {
  const position = await tx.organizationPosition.findUnique({ where: { id: positionId }, select: { id: true, businessUnitId: true, organizationGroupId: true, title: true, status: true, updatedAt: true } });
  if (!position || position.status !== 'ACTIVE') fail('NOT_FOUND', 'Organization position not found.');
  await assertPeopleScope(tx, actor, position);
  return position;
}
...
export async function endPositionAssignment(actor, assignmentId, input, { db = defaultDb } = {}) {
  ...
  return db.$transaction(async (tx) => {
    const current = await tx.positionAssignment.findUnique({ where: { id: assignmentId }, include: { position: { select: { id: true, businessUnitId: true, organizationGroupId: true, title: true, status: true } }, person: { select: { id: true, fullName: true, employeeIdentifier: true } } } });
    if (!current) fail('NOT_FOUND', 'Position assignment not found.');
    await assertPeopleScope(tx, actor, current.position);
