...
function profileWhere(actor) {
  ...
  return { assignments: { some: { ...activeAssignmentWhere(), position: { businessUnitId: { in: ids } } } } };
}

function assignmentWhere(actor) {
  if (!isBusinessUnitScoped(actor)) return {};
  const ids = effectiveBusinessUnitIds(actor);
  return { position: { businessUnitId: { in: ids } } };
}
...
const profileInclude = (actor) => ({
  ...
  assignments: {
    ...
    include: { position: { select: { id: true, title: true, code: true, businessUnit: { select: { id: true, name: true } } } } }
  }
});

function profileDto(person) {
  return {
    id: person.id, fullName: person.fullName, employeeIdentifier: person.employeeIdentifier,
    email: person.email, phone: person.phone, photoUrl: person.photoUrl, status: person.status,
    ...
    certifications: (person.certifications || []).map((row) => ({ id: row.id, name: row.name, issuer: row.issuer, credentialId: row.credentialId, issueDate: dateValue(row.issueDate), expiryDate: dateValue(row.expiryDate), evidenceUrl: row.evidenceUrl, updatedAt: dateValue(row.updatedAt) })),
    assignments: (person.assignments || []).map((row) => ({ id: row.id, startDate: dateValue(row.startDate), endDate: dateValue(row.endDate), type: row.type, position: { id: row.position.id, title: row.position.title, code: row.position.code, businessUnit: { id: row.position.businessUnit.id, name: row.position.businessUnit.name } } }))
  };
}
...
export async function listPeopleProfiles(actor, { query = '' } = {}, { db = defaultDb } = {}) {
  ...
  const people = await db.person.findMany({
    ...
    select: { id: true, fullName: true, employeeIdentifier: true, email: true, phone: true, photoUrl: true, status: true, updatedAt: true }
  });
  return { profiles: people.map((person) => ({ ...person, updatedAt: dateValue(person.updatedAt) })), capabilities: peopleCapabilities(actor) };
}
...
export async function getPersonProfile(actor, personId, { db = defaultDb } = {}) {
  ...
  return { profile: profileDto(person), capabilities: peopleCapabilities(actor) };
}
...
export async function createPersonProfile(actor, input, { db = defaultDb } = {}) {
  ...
  return db.$transaction(async (tx) => {
    ...
    const person = await tx.person.create({ data: { fullName: data.fullName, employeeIdentifier: data.employeeIdentifier, email: data.email, phone: data.phone, photoUrl: data.photoUrl } });
    ...
    await audit(tx, actor.id, person.id, 'CREATE_PERSON_PROFILE', { educationCount: data.educations.length, certificationCount: data.certifications.length });
    return profileDto(complete);
  });
}
...
export async function updatePersonProfile(actor, personId, input, { db = defaultDb } = {}) {
  ...
  return db.$transaction(async (tx) => {
    ...
    const updated = await tx.person.updateMany({ where: { id: personId, status: 'ACTIVE', updatedAt: current.updatedAt }, data: { fullName: data.fullName, employeeIdentifier: data.employeeIdentifier, email: data.email, phone: data.phone, photoUrl: data.photoUrl } });
    ...
    await audit(tx, actor.id, personId, 'UPDATE_PERSON_PROFILE', { educationCount: data.educations.length, certificationCount: data.certifications.length });
    return profileDto(complete);
  });
}
...
export { profileDto, profileInput, profileWhere };
