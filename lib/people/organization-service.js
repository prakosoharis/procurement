...
import { assertPeopleScope } from './scope.js';
...
const positionSelect = {
  id: true, structureId: true, businessUnitId: true, organizationGroupId: true, parentId: true, title: true,
...
function scopeData(target) { return target.organizationGroupId ? { organizationGroupId: target.organizationGroupId } : { businessUnitId: target.businessUnitId }; }
function sameScope(left, right) { return left.businessUnitId === right.businessUnitId && left.organizationGroupId === right.organizationGroupId; }
...
async function scopedStructure(tx, actor, structureId) {
  ...
  await assertPeopleScope(tx, actor, scopeData(structure));
  return structure;
}
...
async function scopedPosition(tx, actor, positionId) {
  ...
  await assertPeopleScope(tx, actor, scopeData(position));
  return position;
}
...
async function updateCurrentPosition(tx, position, data) {
  ...
  if (updated.count !== 1) fail('CONCURRENT_MODIFICATION', 'This position has changed. Reload it before saving.');
...
async function assertUniquePositionCode(tx, { businessUnitId, organizationGroupId, code, excludeId }) {
  if (!code) return;
  const duplicate = await tx.organizationPosition.findFirst({
    where: { ...scopeData({ businessUnitId, organizationGroupId }), code, ...(excludeId ? { id: { not: excludeId } } : {}) },
    select: { id: true }
  });
  if (duplicate) fail('INVALID_INPUT', 'Position code must be unique within this organization scope.');
}
...
function requestedScope(input) {
  const scopeType = input?.scopeType === 'GROUP' ? 'GROUP' : 'BUSINESS_UNIT';
  const scopeId = normalizedText(scopeType === 'GROUP' ? input?.organizationGroupId || input?.scopeId : input?.businessUnitId || input?.scopeId, 'organization scope', { required: true, maxLength: 100 });
  return scopeType === 'GROUP'
    ? { scopeType, organizationGroupId: scopeId, businessUnitId: null }
    : { scopeType, businessUnitId: scopeId, organizationGroupId: null };
}

export async function getOrganizationStructure(actor, input, { db = defaultDb } = {}) {
  if (!peopleCapabilities(actor).canView) fail('FORBIDDEN', 'People access is not available for this role.');
  const target = typeof input === 'string'
    ? { scopeType: 'BUSINESS_UNIT', businessUnitId: input, organizationGroupId: null }
    : requestedScope(input);
  await assertPeopleScope(db, actor, scopeData(target));
  const isGroup = target.scopeType === 'GROUP';
  const [scope, structure] = await Promise.all([
    isGroup
      ? db.organizationGroup.findUnique({ where: { id: target.organizationGroupId }, select: { id: true, name: true } })
      : db.businessUnit.findUnique({ where: { id: target.businessUnitId }, select: { id: true, name: true, groupName: true, industry: true } }),
    db.organizationStructure.findFirst({
      where: { ...scopeData(target), status: 'ACTIVE' },
  ...
  if (!scope) fail('NOT_FOUND', `${isGroup ? 'Organization Group' : 'Business Unit'} not found.`);
  if (!structure) return {
    businessUnit: isGroup ? null : scope,
    scope: { type: target.scopeType, id: scope.id, name: scope.name },
    structure: null,
    nodes: [],
  ...
  return {
    businessUnit: isGroup ? null : scope,
    scope: { type: target.scopeType, id: scope.id, name: scope.name },
    structure: { id: structure.id, name: structure.name, scopeType: structure.scopeType, effectiveDate: structure.effectiveDate, updatedAt: structure.updatedAt },
    nodes,
    capabilities: peopleCapabilities(actor)
  };
}
...
export async function createOrganizationStructure(actor, input, { db = defaultDb } = {}) {
  ...
  const target = requestedScope(input);
  ...
  return db.$transaction(async (tx) => {
    await assertPeopleScope(tx, actor, scopeData(target));
    const scope = target.scopeType === 'GROUP'
      ? await tx.organizationGroup.findUnique({ where: { id: target.organizationGroupId }, select: { id: true } })
      : await tx.businessUnit.findUnique({ where: { id: target.businessUnitId }, select: { id: true } });
    if (!scope) fail('NOT_FOUND', `${target.scopeType === 'GROUP' ? 'Organization Group' : 'Business Unit'} not found.`);
    const active = await tx.organizationStructure.findFirst({ where: { ...scopeData(target), status: 'ACTIVE' }, select: { id: true } });
    if (active) fail('INVALID_TRANSITION', `This ${target.scopeType === 'GROUP' ? 'Organization Group' : 'Business Unit'} already has an active organization structure.`);
    await assertUniquePositionCode(tx, { ...scopeData(target), code: rootCode });
    const structure = await tx.organizationStructure.create({ data: { ...target, name, effectiveDate } });
    const root = await tx.organizationPosition.create({
      data: { structureId: structure.id, ...scopeData(target), title: rootTitle, code: rootCode, description: rootDescription, displayOrder: 0 }
    });
    await audit(tx, actor.id, 'OrganizationStructure', structure.id, 'CREATE_ORGANIZATION_STRUCTURE', { scopeType: target.scopeType, scopeId: target.scopeType === 'GROUP' ? target.organizationGroupId : target.businessUnitId, rootPositionId: root.id, name });
...
export async function createOrganizationPosition(actor, input, { db = defaultDb } = {}) {
  ...
  return db.$transaction(async (tx) => {
    ...
    if (!parentId) {
      ...
      if (parent.structureId !== structure.id || !sameScope(parent, structure)) {
        fail('OUT_OF_SCOPE', 'Parent position must belong to the same organization scope and structure.');
      }
    }
    await assertUniqueActiveSiblingTitle(tx, { structureId, parentId, title });
    await assertUniquePositionCode(tx, { ...scopeData(structure), code });
    const displayOrder = parseOrder(input.displayOrder);
    const position = await tx.organizationPosition.create({
      data: { structureId, ...scopeData(structure), parentId, title, code, description, displayOrder: displayOrder ?? await nextSiblingOrder(tx, structureId, parentId) }
    });
    await audit(tx, actor.id, 'OrganizationPosition', position.id, 'CREATE_ORGANIZATION_POSITION', { structureId, parentId, title, code, scopeType: structure.scopeType });
    return position;
  });
}
...
export async function updateOrganizationPosition(actor, positionId, input, { db = defaultDb } = {}) {
  ...
  return db.$transaction(async (tx) => {
    ...
    await assertUniquePositionCode(tx, { ...scopeData(position), code, excludeId: position.id });
...
export async function moveOrganizationPosition(actor, positionId, input, { db = defaultDb } = {}) {
  ...
  return db.$transaction(async (tx) => {
    ...
    if (newParent.structureId !== position.structureId || !sameScope(newParent, position)) {
      fail('OUT_OF_SCOPE', 'A position can be moved only within its organization scope and structure.');
    }
    await assertUniqueActiveSiblingTitle(tx, { structureId: position.structureId, parentId: newParent.id, title: position.title, excludeId: position.id });
