...
import { googleDriveClient, googleDriveConfigured } from './google-drive';
...
// Check the application configuration and the saved Drive connection before a
// document record is created. This prevents an upload failure from leaving an
// empty Draft document in the compliance matrix.
export async function assertStorageReady() {
  if (!useGoogleDrive()) return;
  if (!googleDriveConfigured()) {
    throw new StorageConfigurationError('Google Drive belum dikonfigurasi di environment Production.');
  }
  try {
