...
import { createGoogleDriveResumableUpload, getGoogleDriveFileMetadata, googleDriveClient, googleDriveConfigured } from './google-drive.js';
...
export async function uploadObject({ key, body, contentType, googleDriveParentId, googleDriveFileName }) {
...
export async function createDirectUploadSession({ fileName, fileSize, contentType, googleDriveParentId, appProperties }) {
  await assertStorageReady();
  if (!isGoogleDriveStorage()) throw new StorageConfigurationError('Direct upload hanya tersedia untuk Google Drive.');
  const { folderId } = await googleDriveClient();
  return createGoogleDriveResumableUpload({
    name: fileName,
    parentId: googleDriveParentId || folderId,
    contentType,
...
export async function getDirectUploadMetadata(fileId) {
  await assertStorageReady();
  if (!isGoogleDriveStorage()) throw new StorageConfigurationError('Direct upload hanya tersedia untuk Google Drive.');
  return getGoogleDriveFileMetadata(fileId);
}
