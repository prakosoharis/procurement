...
// Templates are blank starting points, not any Business Unit's governing
// document, so they live outside the SOP tree entirely -- nobody browsing
// Drive should mistake a template for an issued policy.
export async function ensureTemplateFolder({ documentTypeCode, create = true, ...context }) {
  if (!documentTypeCode) throw new Error("Document type code is required.");
  const resolvedContext = await driveContext(context);
  const resolved = await resolveGoogleDriveFolderPath({
    ...resolvedContext,
    segments: ["Template SOP", documentTypeCode],
    create,
  });
  if (!resolved.folderId) throw new Error("Template Google Drive folder could not be resolved.");
  return resolved;
}
