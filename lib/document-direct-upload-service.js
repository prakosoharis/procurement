...
export async function prepareDocumentDirectUpload(actor, input, {
  ...
  const [existing, owner, reviewer, businessUnit, active] = await Promise.all([
    db.sopDocument.findFirst({ where: { businessUnitId, documentTypeId, title, status: { not: 'ARCHIVED' } }, select: { id: true } }),
    ...
    db.googleDriveUploadSession.findFirst({
      where: { purpose: 'CREATE_DOCUMENT', status: { in: ACTIVE_STATUSES }, businessUnitId, documentTypeId, title },
    })
  ]);
  if (existing) throw new DirectUploadError('CONFLICT', 'Dokumen dengan nama ini sudah ada untuk Business Unit tersebut. Gunakan update versi.', 409);
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      if (current.purpose === 'CREATE_DOCUMENT') {
        const existing = await tx.sopDocument.findFirst({
          where: { businessUnitId: current.businessUnitId, documentTypeId: current.documentTypeId, title: current.title, status: { not: 'ARCHIVED' } },
          select: { id: true }
        });
        if (existing) throw new DirectUploadError('CONFLICT', 'Dokumen dengan nama ini sudah ada untuk Business Unit tersebut.', 409);
