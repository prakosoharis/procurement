...
async function loadPdfJs() {
  ...
  pdfJsModulePromise ??= import("pdfjs-dist/legacy/build/pdf.mjs").then((pdfjs) => {
    // pdfjs-dist's default workerSrc ("./pdf.worker.mjs") is resolved
    // relative to its OWN module location at import time. That breaks under
    // a bundler that relocates the module (Trigger.dev's esbuild task
    // bundle copies this file into .trigger/tmp/build-<hash>/, where no
    // sibling pdf.worker.mjs exists), throwing "Setting up fake worker
    // failed: Cannot find module ... pdf.worker.mjs". Resolving through
    // Node's own module resolution instead of a bundler-relative path
    // finds the real on-disk file regardless of how the importing module
    // was bundled.
