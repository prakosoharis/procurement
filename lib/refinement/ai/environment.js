...
export function validateEnvironment(environment = process.env, { requireAiProviders = false } = {}) {
  ...
  if (provider === "claude-max-agent") {
    errors.push("AI_PROVIDER=claude-max-agent has no deployable runtime: Claude subscription credentials may not be used to serve application users. Use AI_PROVIDER=anthropic-api.");
  } else if (provider !== "anthropic-api") {
    errors.push(`AI_PROVIDER "${provider}" is not supported. Supported: anthropic-api.`);
