...
export function validateEnvironment(environment = process.env, { requireAiProviders = false } = {}) {
  ...
  const supportedProviders = ["anthropic-api", "zai"];
  ...
  if (provider === "claude-max-agent") {
    errors.push("AI_PROVIDER=claude-max-agent has no deployable runtime: Claude subscription credentials may not be used to serve application users. Use AI_PROVIDER=anthropic-api or AI_PROVIDER=zai.");
