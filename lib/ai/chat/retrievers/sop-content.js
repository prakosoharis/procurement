...
export async function retrieveSopContent({ actor, db, question, limit = 12 }) {
  ...
  // Mirrors sopDocumentScopeWhere: a Business Unit viewer sees their own
  // Business Unit's documents plus those issued by a Group that contains it.
  const scopeClause = isBusinessUnitScoped(actor)
    ? Prisma.sql`AND (
        d."businessUnitId" = ANY(${effectiveBusinessUnitIds(actor)})
        OR d."organizationGroupId" IN (
          SELECT "organizationGroupId" FROM "BusinessUnit"
          WHERE id = ANY(${effectiveBusinessUnitIds(actor)}) AND "organizationGroupId" IS NOT NULL
        )
      )`
    : Prisma.empty;
  ...
  const rows = await db.$queryRaw`
    SELECT s.id, s."pageNumber", s.text, d.title, d."scopeType",
           COALESCE(bu.name, og.name) AS "ownerName"
    FROM "SopSection" s
    JOIN "SopVersion" v ON v.id = s."sopVersionId"
    JOIN "SopDocument" d ON d.id = v."sopDocumentId"
    -- LEFT: a Group document has no Business Unit, and an inner join here
    -- silently dropped every one of them from search results.
    LEFT JOIN "BusinessUnit" bu ON bu.id = d."businessUnitId"
    LEFT JOIN "OrganizationGroup" og ON og.id = d."organizationGroupId"
  ...
  return {
    ...
    records: rows.map((row) => ({
      ...
      // "issuer" rather than businessUnit now that a document can come from a
      // Group, so the model never reports a Group as if it were a BU.
      issuer: row.ownerName,
      issuerLevel: row.scopeType === 'GROUP' ? 'GROUP' : 'BUSINESS_UNIT',
