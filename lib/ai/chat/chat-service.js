...
export async function answerChatQuestion({
  ...
  if (!classification.inScope) {
    ...
    return { answer: OUT_OF_SCOPE_ANSWER, dataAvailable: false, references: [], inScope: false, topics: [], mode: 'OUT_OF_SCOPE' };
  }

  const results = await retrieveForTopics({ actor, db, topics: classification.topics });

  // Deterministic mode answers straight from the retrieved records. No provider
  // is called, and the response is marked so the interface never presents it
  // as AI analysis.
  if (config.chatMode === 'data-summary') {
    return { ...answerFromData({ question: text, results }), inScope: true, topics: classification.topics };
  }
  ...
  // The schema guarantees a reference's SHAPE, not that it points at a record
  // the model actually saw. Verify every citation against what was truly
  // retrieved before it reaches the user.
