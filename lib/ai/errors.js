...
// Governance routes report through {ok,error:{code,message}}. Convert with the
// safe user message so an internal provider detail is never serialized.
export function asGovernanceError(error) {
  if (!isAiServiceError(error)) return error;
  const converted = new Error(error.userMessage);
  converted.name = 'GovernanceError';
