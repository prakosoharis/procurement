...
| --- | --- | --- |
| Product shell | At `REF-S0-T1`/`T2` time, the approved interface was the static hub at `/` and `/hub/[page]`, rendered from `procurement-governance-hub (1).html` / `public/procurement-governance-hub.html`. Every hub page has since migrated to its own dedicated React route under `app/hub/<page>/` (Home, Repository, Refinement, Calendar, Engagement, Insights, People, Directory, Requests); the dynamic iframe route and its Blob-upload bridge component no longer exist. The static asset files are kept on disk only as the approved visual reference each React page was ported against. | Keep the menu structure and visual behaviour. Refinement work must stay inside the existing Home, Submissions, Repository, Refinement, Calendar, Engagement, Insights, and Directory menus -- now implemented as React pages rather than static markup in an iframe. |
