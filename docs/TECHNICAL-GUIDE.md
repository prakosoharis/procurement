...
| `GEMINI_REASONING_EFFORT` | `low` (default), `medium`, or `high`. Gemini 3 always thinks before answering and thinking tokens are billed against `max_tokens`; sending no effort at all made the free tier return `finish_reason: "length"` with zero output tokens. Measured on the free tier: `low` answered correctly 3/3 including full grounded chat prompts, `medium` returned HTTP 503 on 2 of 3 attempts, and `minimal` is rejected with HTTP 400 by this model. Raise it only on a paid tier. |
